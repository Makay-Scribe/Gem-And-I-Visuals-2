...
void main() {
    ...
    // ** THE FIX IS HERE: Implement the settling phase logic. **
    // For the first second of the simulation, gently fade in the pressure force.
    float settleDuration = 1.0; 
    float settleFactor = smoothstep(0.0, settleDuration, u_time);

    // Combine Forces & Integrate
    // The volatile pressureForce is scaled by the settleFactor, while viscosity and external forces are applied normally.
...
void main() {
    ...
    // Combine Forces & Integrate
    // The main "active" forces are scaled by the settleFactor to ensure a static start.
    vec3 totalForce = (pressureForce * settleFactor) + viscosityForce;
    vec3 acceleration = vec3(0.0);
    ...
    // ** THE FIX IS HERE: Gravity is also scaled by the settle factor. **
